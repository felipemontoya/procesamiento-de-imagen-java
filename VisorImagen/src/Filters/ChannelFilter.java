 ...
 * @author Arzobispo
 */
public class ChannelFilter extends FilterPipeObject {
    ...
    @Override
     public boolean InternalUpdate(){
        ...
        int[] a_channel = new int[data.getnCanales()];
       ...
       for(int i = 0;i<data.getHeight();i++){
            for(int j = 0;j<data.getWidth();j++){
                for(int k = 0 ;k<data.getnCanales();k++){
                    data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + k] *= a_channel[k];
                }
          }
        }
