      ...
      for(int i = 0;i<data.getHeight();i++){
            for(int j = 0;j<data.getWidth();j++){
                for(int k = 0 ;k<data.getnCanales();k++){
                    data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + k] = (byte)Math.pow(dataIn.getImageData().bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + k],1/gamma);
                }
          }
        }
