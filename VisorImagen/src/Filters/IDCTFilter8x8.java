...
for (int offsetY = data.getHeight()-8; offsetY >= 0 ; offsetY-=8){

    for (i = 0; i < N; i++){// sumatorias
          for (j = 0; j < M; j++){
            outputR[i][j]=(byte)(data.bytesImage[((j + offsetY) * data.getWidthStep() + i + offsetX ) * nChannels + 0]);
            outputG[i][j]=(byte)(data.bytesImage[((j + offsetY) * data.getWidthStep() + i + offsetX ) * nChannels + 1]);
            outputB[i][j]=(byte)(data.bytesImage[((j + offsetY) * data.getWidthStep() + i + offsetX ) * nChannels + 2]);
        }
    }
    ...
    for (i = 0; i < N; i++){// sumatorias
          for (j = 0; j < M; j++){
           data.bytesImage[((j + offsetY) * data.getWidthStep() + i + offsetX ) * nChannels + 0] = generateByte((int)Math.round(outputR[i][j]));
            data.bytesImage[((j + offsetY) * data.getWidthStep() + i + offsetX ) * nChannels + 1] = generateByte((int)Math.round(outputG[i][j]));
            data.bytesImage[((j + offsetY) * data.getWidthStep() + i + offsetX ) * nChannels + 2] = generateByte((int)Math.round(outputB[i][j]));
        }
    }
    }
   }
