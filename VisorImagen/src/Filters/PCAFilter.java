 ...
 * @author Jhon
 */
public class PCAFilter extends FilterPipeObject{

    private String name;

    public PCAFilter(String name) {
        super("Principal Component Analysis Transformation " + name);
        this.name = name;
        ...
        for (int i = 0; i < data.getWidth(); i++)
            for (int j = 0; j < data.getHeight(); j++)
                for (int k = 0; k < data.getnCanales(); k++)
                    {
                        mean[k] += BytesToInt(data.bytesImage[(i + (j*data.getWidth()))* 3 + k]);
                    }

        double size = data.getWidth()*data.getHeight();
        for (int k = 0; k < data.getnCanales(); k++)
            mean[k] /= size;

        // center the data
        double centered[][][] =  new double[data.getWidth()][data.getHeight()][data.getnCanales()];
        for (int i = 0; i < data.getWidth(); i++)
            for (int j = 0; j < data.getHeight(); j++)
                for (int k = 0; k < data.getnCanales(); k++)
                    {
                        centered[i][j][k] = BytesToInt(data.bytesImage[(i + (j*data.getWidth()))* 3 + k]) - mean[k];
                    }

        // calculate covariances
        double covMat[][] = new double[data.getnCanales()][data.getnCanales()];
        for (int x = 0; x < data.getnCanales(); x++)
            for (int y = 0; y < data.getnCanales(); y++)
            {
                if (y < x)
                    continue;
                double sum = 0;
                for (int i = 0; i < data.getWidth(); i++)
                    for (int j = 0; j < data.getHeight(); j++)
                        sum += centered[i][j][x]*centered[i][j][y];
                covMat[x][y] = sum / size ;
...
//         for (int x = 0; x < data.getnCanales(); x++){
//            for (int y = 0; y < data.getnCanales(); y++)
//                 System.out.print(" " + covMat[x][y]);
//             System.out.println("");
//        }

        // get eigenvectors using Jama
        Matrix mat = new Matrix(covMat);
        EigenvalueDecomposition eig = new EigenvalueDecomposition(mat);
        Matrix eigenVectors = eig.getV();

        double eigV[][] = eigenVectors.getArray();



        // project data in eigenspace
        double projected[][][] =  new double[data.getWidth()][data.getHeight()][data.getnCanales()];
        for (int i = 0; i < data.getWidth(); i++)
            for (int j = 0; j < data.getHeight(); j++)
                for (int k = 0; k < data.getnCanales(); k++)
                    {
                        projected[i][j][k] = centered[i][j][0]*eigV[k][0] +centered[i][j][1]*eigV[k][1] + centered[i][j][2]*eigV[k][2] + mean[k];
                    }
        // Copy the data to output
        for (int i = 0; i < data.getWidth(); i++)
            for (int j = 0; j < data.getHeight(); j++)
            {
                data.bytesImage[(i + (j*data.getWidth()))*3] = DoubleToByte(projected[i][j][0]);
                data.bytesImage[(i + (j*data.getWidth()))*3+1] = DoubleToByte(projected[i][j][1]);
                data.bytesImage[(i + (j*data.getWidth()))*3+2] = DoubleToByte(projected[i][j][2]);
            }
