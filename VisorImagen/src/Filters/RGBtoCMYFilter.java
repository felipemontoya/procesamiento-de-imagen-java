 ...
 * @author Jhon
 */
public class RGBtoCMYFilter extends FilterPipeObject{

    private String name;

    public RGBtoCMYFilter(String name) {
     ...
     public boolean InternalUpdate(){
       ...
       for(int i = 0;i<data.getHeight();i++){
            for(int j = 0;j<data.getWidth();j++){
                if(RGB){
                  data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 0]= spaceCMY(data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 0]);
                  data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 1]= spaceCMY(data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 1]);
                  data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 2]= spaceCMY(data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 2]);
                }else{
                  data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 2]= spaceCMY(data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 2]);
                  data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 1]= spaceCMY(data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 1]);
                  data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 0]= spaceCMY(data.bytesImage[(i * data.getWidthStep() + j ) * data.getnCanales() + 0]);
    ...
    private byte spaceCMY(byte R){
        float y;
        int RR=BytesToInt(R);
        return generateByte(255-RR);
    }
