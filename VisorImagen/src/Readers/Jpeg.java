...
//      jpg = new JpegEncoder(image, Quality, dataOut);
  //    jpg.Compress();
    ...
    public void createJPG(ImageData image,String name){
    FileOutputStream dataOut = null;
    File outFile;
    JpegEncoder jpg;
    String string = "";
    ...
    outFile = new File(name);
      ...
      try {
        dataOut = new FileOutputStream(outFile);
          ...
          System.out.println("Salio algo mal al guardar el archivo...");
      }
...
class JpegEncoder extends Frame {
  ...
  public JpegEncoder(ImageData image, int quality, OutputStream out) {
...
class JpegInfo  {
  ...
  public JpegInfo(ImageData image) {
  ...
  private void getYCCArray()  {
    ...
    for (y = 0; y < imageHeight; ++y) {
      for (x = 0; x < imageWidth; ++x) {
        r = BytesToInt(imageobj.bytesImage[(y * imageWidth + x)*3 + 0]);
        g = BytesToInt(imageobj.bytesImage[(y * imageWidth + x)*3 + 1]);
        b=  BytesToInt(imageobj.bytesImage[(y * imageWidth + x)*3 + 2]);
