...
public class cTest_puntoJOGL extends JFrame implements GLEventListener
{
   ...
   // Constructor
    public cTest_puntoJOGL(ImageData i)
    {
    	...
    	altura = (int)dimensionPantalla.height;
    	anchura = (int)dimensionPantalla.width;
    ...
    public void display(GLAutoDrawable drawable)
    {
		...
		for(int i = 0;i<im.getHeight();i++){
                    for(int j=0;j<im.getWidth();j++){
                       ...
                       gl.glColor3f(color(datosb[i][j][0]), color(datosb[i][j][1]), color(datosb[i][j][2]));
                       gl.glVertex2i(j, i);
